...
for(i=0;i<size;i+=PGSIZE)
{
	...
	if(((uint)(*pte) &  PTE_U)==0)
	{
		for(x=0;x<PGSIZE;x+=4)
		{
			memmove((char*)buffer+x,&guardPageFill,4);
		}
//		int* guardPtr=(int*)buffer;
//		cprintf("Guard page at: 0x%x \n",guardPtr);
	}
	buffer+=PGSIZE;
...
int thread_cond_wait(void *chan, lock_t *lk) {

  struct proc *p;
  struct proc *curproc = myproc();
  acquire(&ptable.lock);
  xchg(&lk->locked,0); //unlock

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {

    if(curproc->pid == p->pid && curproc->isthread == 1) {
	 p->state = SLEEPING;
	 *(int*)chan = 0;
	 sched();
	}
  ...
  release(&ptable.lock);
  while(xchg(&lk->locked,1)!=0);//lock
  return 0;
}

int thread_cond_signal(void* chan) {
  acquire(&ptable.lock);
  struct proc *p;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if(p->state == SLEEPING && (int)p->chan == *(int*)chan && p->isthread == 1) {
	 p->state = RUNNABLE;
	}
  }
  release(&ptable.lock);
